...
test('macro contract distinguishes unknown and known zero; rejects non-finite, negative, excessive values', () => {
  assert.deepEqual(validateMacros({ protein: 0, netCarbs: null, fat: 10.25 }), { protein: 0, netCarbs: null, fat: 10.25 });
  for (const protein of [-1, NaN, Infinity, 100001, '2']) assert.throws(() => validateMacros({ protein, netCarbs: null, fat: null }));
  ...
  const macros = { protein: 6, netCarbs: 0.5, fat: 5 };
...
test('text estimate sends only the one food, requests total-portion grams, and validates the reply', async () => {
  ...
  const result = { protein: 12, netCarbs: 1, fat: 10 };
  let calls = 0;
  const client = { responses: { parse: async (request: Record<string, unknown>) => {
    ...
    assert.match(String(request.instructions), /minus fiber/);
