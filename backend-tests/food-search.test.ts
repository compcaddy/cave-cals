...
test('Premier macros use the selected serving and preserve unknown fiber, zero, and invalid nutrients', () => {
  function normalize(serving: Record<string, unknown>) {
    return normalizeFoods({ foods_search: { total_results: '1', results: { food: { ...food, servings: { serving: {
