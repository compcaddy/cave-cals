# Easiest Calorie Counter 2

A native, iPhone-only calorie logger built with SwiftUI and SwiftData. Supports iOS 17 and later. No app account, backend, API key, third-party Swift dependencies, or subscriptions.

## Open and run

Open `EasiestCalorieCounter.xcodeproj`, select the **EasiestCalorieCounter** scheme, choose an iPhone simulator, and run. The project is already generated; you do not need a project-generation tool to open it.

- Bundle identifier: `com.phil.EasiestCalorieCounter2`
- Deployment target: iOS 17.0
- Supported device family: iPhone only, portrait
- App title: Easiest Calories
- Simulator builds use local persistence and do not need signing.
...
- Current-week selector, past-week swiping/buttons, disabled future days, day status, and historical editing.
...
- Always-visible search/calorie field and barcode button.
...
- Open Food Facts food search and camera barcode lookup, manual barcode fallback, and persistent local barcode overrides.
...
- Dynamic Type, VoiceOver labels, system light/dark appearance, and a simple utility icon.

## iCloud setup and verification

In Xcode's Signing & Capabilities, select your team and enable **iCloud → CloudKit**, using `iCloud.com.phil.EasiestCalorieCounter2`. The entitlements and remote-notification background mode are included. Allow Xcode to create/update provisioning profiles. The project contains no Apple account credentials.
...
Product data and attribution: [Open Food Facts](https://world.openfoodfacts.org), [API documentation](https://openfoodfacts.github.io/openfoodfacts-server/api/), [ODbL](https://opendatacommons.org/licenses/odbl/1-0/). Restaurant/generic food coverage depends on what the provider contains. API requests send the query or barcode; profile and diary data are not sent to Open Food Facts. No analytics SDK is included.
...
```sh
xcodebuild test -project EasiestCalorieCounter.xcodeproj \
  -scheme EasiestCalorieCounter \
...
V1 intentionally excludes weight, macros, water, exercise, meal categories, voice/photo AI, and future meal planning.
