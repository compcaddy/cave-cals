...
export async function identify(upload: Upload, bytes: Buffer, client = new OpenAI({ apiKey: required('OPENAI_API_KEY'), timeout: 90000, maxRetries: 0 })): Promise<FoodResult> {
  ...
  if (upload.kind === 'image') {
    ...
    content.push({ type: 'input_text', text: 'Identify the food and estimate calories for the portions shown. If this is a label or menu screenshot, use its readable nutrition and serving information.' });
  } else {
